...
void Cpu_::LoadInc(ByteReg& dst, RegPair src) {
    dst.v = this->mmu->Read(src.Addr());
    src.Inc();
}

void Cpu_::LoadInc(RegPair dst, ByteReg src) {
    this->mmu->Write(dst.Addr(), src.v);
    dst.Inc();
}

void Cpu_::LoadDec(ByteReg& dst, RegPair src) {
    dst.v = this->mmu->Read(src.Addr());
    src.Dec();
}

void Cpu_::LoadDec(RegPair dst, ByteReg src) {
    this->mmu->Write(dst.Addr(), src.v);
    dst.Dec();
}

//16-bit transfer
void Cpu_::Load(RegPair dst, u16 imm) {
    dst.h = bit::High(imm);
    dst.l = bit::Low(imm);
}

void Cpu_::Load(WordReg& dst, u16 imm) { dst.v = imm; }

void Cpu_::Load(WordReg& dst, RegPair src) { dst.v = src.Addr(); }

void Cpu_::Push(RegPair src) {
    this->mmu->Write(--this->sp.v, src.h);
    this->mmu->Write(--this->sp.v, src.l);
}

void Cpu_::Pop(RegPair dst) {
    dst.l = this->mmu->Read(this->sp.v++);
    dst.h = this->mmu->Read(this->sp.v++);
}

void Cpu_::Load(RegPair dst, WordReg src, s8 imm) {
