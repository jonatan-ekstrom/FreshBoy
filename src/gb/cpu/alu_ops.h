...
/* Class containing the implementation of all ALU-related operations. */
class AluOps {
public:
    explicit AluOps(Cpu_& cpu);
    ...
    static void Inc(RegPair rp); // rp := rp + 1
    static void Inc(WordReg& reg); // reg := reg + 1

    static void Dec(RegPair rp); // rp := rp - 1
    static void Dec(WordReg& reg); // reg := reg - 1
private:
    Cpu_& cpu;
};
